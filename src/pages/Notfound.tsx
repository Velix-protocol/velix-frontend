...
export default function Notfound({ isNested = false }: { isNested?: boolean }) {
  ...
  return (
    <Section className="px-5 fixed top-0 left-0 right-0 h-screen bg-velix-black">
      {!isNested && (
        <div className="flex items-center justify-between py-8">
          <div className="flex items-center justify-betwee space-x-32 text-base">
            <Link to="/">
              <VelixPrimaryBlackLogo className="fill-white w-[4.25rem] h-4" />
            </Link>
      ...
      <div className="relative flex flex-col items-center w-fit mt-28 mx-auto">
        ...
        <h2 className="text-2xl md:text-4xl text-center text-white font-bold font-space-grotesk mb-5 md:mb-10">
          Page not found
        </h2>
        <Button
          onClick={() => navigate("/", { replace: true })}
          className="flex bg-white text-velix-black hover:bg-white font-space-grotesk px-8 font-bold text-center items-center"
        >
          Go home
        </Button>
      </div>
